...
/**
 * Contains functions which return the specified value for a given id.
 */
class ValueReader {
  /**
   ...
   * @author hg
   * @param {number} id The id of the wanted text.
      ...
      if (element.id == id) {
        if (!element.translateable) {
          ...
          for (var l in element.languages) {
            if (l == language) {
              //Otherwise it trys to access element.languages.l (instead of ...de for instance)
   ...
   * @author hg
   * @param {number} id The id of the wanted image.
   */
  getImage(id) {
   ...
   * @author hg
   * @param {number} id The id of the wanted color.
   */
  getColor(id) {
   ...
   * @author hg
   * @param {number} id The id of the wanted font.
   */
  getFont(id) {
    var file = fs.readFileSync('../../fonts.json');
    var jsonFile = JSON.parse(file);
    for (var i = 0; i < jsonFile.content.length; i++) {
      let element = jsonFile.content[i];
      if (element.id == id) {
...
export function getText(id, language) {
  var file = fs.readFileSync('./app/config/text.json');
  var jsonFile = JSON.parse(file);
